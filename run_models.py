...
def run_models(base_directory, train_nt, test_nt, sampling_rate=2, seq_len=60, pre_len=10, repeat=False, is_continuous=False):
    ...
    be_arima = time.time()
    arima_header, arima_test, arima_result, count_invalid, total = arima_sampling(train, test, seq_len=seq_len, pre_len=pre_len, repeat=repeat, is_continuous=is_continuous, sampling_rate=sampling_rate, p=1, d=0, q=0)
    ...
    file_name = 'Days-20-end-10-sam-{}-seq-{}-pre-{}-repeat-{}-is-continous-{}.csv'.format(
        sampling_rate,
        seq_len,
        pre_len,
        repeat,
